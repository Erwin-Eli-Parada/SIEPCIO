...
if (isset($_POST['claveMunicipio'])) {
    ...
    if ($claveMunicipio === '') {
        $sql = "SELECT servicio_salud, sum(porcentaje) as porcentaje FROM servicio_salud ss JOIN municipio m ON ss.clave_mun = m.clave_mun WHERE m.tipo = 'Indigena' group by servicio_salud";
    } else {
